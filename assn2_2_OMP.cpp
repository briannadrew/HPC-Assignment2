...
// Adapted From: https://www.geeksforgeeks.org/runge-kutta-4th-order-method-solve-differential-equation/
...
void rungeKutta(float x, float h, int n)
{
  ...
  for (int i = 1; i <= n; i++) {
    ...
    // work-around for barrier not being allowed in for loop
...
int main()
{
  ...
  // call function to calculate 4th order runge-kutta
  rungeKutta(x, h, n);
